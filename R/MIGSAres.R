...
MIGSAres.data.table <- function(x, genes_rank=list(), bp_param=bpparam(),
    ... ) {
    migsa_res_summ <- summary_from_all(x);
    
    .Object <- new("MIGSAres", migsa_res_all=x,
                    migsa_res_summary=migsa_res_summ,
                    genes_rank=genes_rank, bp_param=bp_param);
    return(.Object);
}
...
# if there is not enr_cutoff set then it puts the default one
