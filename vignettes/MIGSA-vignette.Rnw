...
perform both SEA and GSEA in an integrative manner by means of the best 
available methods, i.e. \pkg{dEnricher} and \pkg{mGSZ} respectively.
...
same subjects studied over several different platforms such as the TCGA 
project, among others, thus making possible the study and comparison in a 
massive way of these cohorts, not only between them but also against our own 
population. This unprecedent oportunity allows to search for common functional 
patterns, or more interestingly, particular patterns of our experiment in 
question. This analysis methodology has not yet been exploited since at present 
there is no tool that allows this type of exploration, thus leaving aside a lot 
of valuable information that might lead to research topics.
...
shown in \citep{rodriguez2016improving} in order to gain as much biological 
insight as possible, both SEA and GSEA must be performed, this strategy is know 
as Integrative Functional Analysis (IFA), which integrates into the same 
analysis the dEnricher \citep{dEnricher} and mGSZ \citep{mGSZ} methods.
...
speedup.
\\Lets test it!
\\Note that we are using Multicore param as I am testing under Linux.
...
<<echo=FALSE>>=
mGSZspeedup <- loadMIGSAdata("mGSZspeedup");
...
## There will be 10 differentialy expressed genes.
nDeGenes <- 10;
...
# _sep_ is the string used to separate each gene
@
...
## another way of generating the same MIGSAinput would be setting the 
## gene sets individually to each IGSAinput:
igsaInput1 <- IGSAinput(name="igsaInput1", expr_data=exprData1, 
    fit_options=fitOpts, gene_sets_list=list(myGSs1, myGSs2));
igsaInput2 <- IGSAinput(name="igsaInput2", expr_data=exprData2, 
    fit_options=fitOpts, gene_sets_list=list(myGSs1, myGSs2));
...
igsaInput1 <- IGSAinput(name="igsaInput1", expr_data=exprData1, 
    fit_options=fitOpts, gene_sets_list=list(myGSs1));
igsaInput2 <- IGSAinput(name="igsaInput2", expr_data=exprData2, 
    fit_options=fitOpts, gene_sets_list=list(myGSs2));
...
In this section we are going to demostrate \pkg{MIGSA}'s utility by analyzing 
...
\\ \textbf{NOTE:} An internet connection is required to download these datasets.
...
geneExprIgsaInput <- IGSAinput(
    ...
    expr_data=geneExpr,
    fit_options=fitOpts,
    # with this treat we will get around 5% diferentially expressed genes
...
# lets create our MIGSAinput object
...
# diferentially expressed genes for each experiment
...
set.seed(8818);
pbcmcMigsaRes <- MIGSA(migsaInput);
...
bcMigsaRes <- setEnrCutoff(bcMigsaRes, as.numeric(NA));
@
...
## Lets see enrichment heatmap
## i.e. a heatmap of binary data (enriched/not enriched)
aux <- migsaHeatmap(bcMigsaRes, trace="none", dendrogram="column", key=FALSE, 
    keysize=0.4, srtCol=45, xlab="Yellow: enriched    Orange: not enriched");
@
...
## In this heatmap we can see a high number of gene sets that are being 
...
## i.e. a heatmap showing the number of datasets in which each gene (columns) 
...
## Well, we could continue exploring them, however, at the first heatmap we can 
## see that tcga datasets are defining a separate cluster, this is caused by a 
## big group of gene sets that seem to be enriched mainly by tcga.
## Lets explore them:
...
## i.e. a barplot of the number of gene sets in which each gene contributed to 
## enrich.
mostEnrichedGenes <- genesBarplot(tcgaExclusive, gsFilter=12.45);
@
...
genes of intereset, we can filter our results having the gene sets that were 
...
<<MIGSAres exploring10, eval=FALSE>>=
...
<<MIGSAres exploring11, eval=TRUE>>=
dim(intGenesMigsa);
