...
\textit{pbcmcData.RData} which can be loaded using loadMIGSAdata("pbcmcData").
}
...
pbcmcData <- lapply(libNames, function(actLibName) {
    ...
    # we recommend working allways with Entrez IDs, lets match them with 
...
And lets check it is the same data.
<<validateData, eval=TRUE>>=
library(MIGSA);

pbcmcData2 <- loadMIGSAdata("pbcmcData");
all.equal(pbcmcData, pbcmcData2);
@
